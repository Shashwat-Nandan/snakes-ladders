...
import { BOARD_SIZE, PLAYER_COLORS, PLAYER_GLOWS, BOARD_ORDER } from "../game/constants";
...
function CopyButton({ text }) {
  ...
  async function handleCopy() {
    ...
    setTimeout(() => setLabel("Copy link"), 1200);
  }
...
function Board({ gameState }) {
  const boardSquares = BOARD_ORDER;
...
function GamePanel({ gameState, rollFeedback, busy, onRoll, onNewGame }) {
      ...
      <aside className="left-column">
        <div className="panel spotlight-panel">
          <div className="panel-header">
            ...
            <a href={shareLink} className="text-link">
              shareable link
            </a>
          </div>
        ...
        <div className="panel control-panel">
          ...
          {winner ? (
            <button type="button" className="primary-button giant-button" onClick={onNewGame}>
              Leave Room
            </button>
          ) : (
...
export function HomePage() {
  const { authError, booting, busy, gameState, roomIdFromUrl, rollFeedback, createGame, joinExistingGame, roll, clearError, resetGame } =
    useGameClient();
  ...
  return (
    <main className="page-shell">
      ...
      {gameState ? (
        <GamePanel gameState={gameState} rollFeedback={rollFeedback} busy={busy} onRoll={roll} onNewGame={resetGame} />
      ) : (
        <AuthPanel
